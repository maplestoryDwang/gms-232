# Yellow Seal in Floor 2 of Oz Tower
...
if instance is not None:
    ...
    if not instance.hasProperty("y"):

        reactor = field.getLifeByTemplateId(OzConstants.YELLOW_REACTOR)
        ...
        if reactor.getState() < 7:
            ...
            if ("v" in colour or "y" in colour) and number > int(reactor.getState()) + 1:
                ...
                instance.addProperty("y", True)
                instance.addProperty(OzConstants.F2_SEALS_CLEARED, int(instance.getProperty(OzConstants.F2_SEALS_CLEARED)) + 1)
                instance.broadcast(WvsContext.weatherEffectNotice(WeatherEffNoticeType.Oz_Alicia, "You have tricked the Yellow seal!", 7000))
