...
bossinfos = {
    MuLungDojoF1 : [[9305600, 5200000], [0, 0, 0], 1], # Mano
    ...
    MuLungDojoF74 : [[9601009, 3255000000000], [0, 0, 0], 1], # alang
    MuLungDojoF75 : [[9601010, 3570000000000], [0, 0, 0], 1], # bonsun
    MuLungDojoF76 : [[9601011, 3885000000000], [0, 0, 0], 1], # wukong
    MuLungDojoF77 : [[9601012, 4200000000000], [0, 0, 0], 1], # sondal
    MuLungDojoF78 : [[9601013, 4550000000000], [0, 0, 0], 1], # gold dragon
    MuLungDojoF79 : [[9601014, 4900000000000], [0, 0, 0], 1], # Red tiger
    MuLungDojoF80 : [[9305339, 5250000000000], [0, 0, 0], 1] # Mu Gong
}
stage = ((sm.getFieldID() % 10000) / 100)

if not "point" in sm.getQRValue(DOJO_QUEST):
    sm.startQuest(DOJO_QUEST)
    sm.setQRValue(DOJO_QUEST, "point", "0")

bossinfo = bossinfos[sm.getFieldID()]
if field.getProperty("curMobCount") < bossinfo[2]:
    ...
    pointAmount = sm.getQRValue(DOJO_QUEST, "point")
    pointsAmount = int(pointAmount)
    pointsToBeAdded = stage * 100
    totalPoints = pointsAmount + pointsToBeAdded
    sm.setQRValue(3887 , "point", str(totalPoints))
    sm.progressMessageFont(1, 14, 4, 0, "You have gained " + str(pointsToBeAdded) + " dojo points. (total: " + str(totalPoints) + ")")
    sm.showEffectToField(WzConstants.EFFECT_DOJO_CLEAR, 0)
    field.setProperty("cleared", True)
    sm.setQRValue(DOJO_QUEST, "floor", str(((sm.getFieldID() % 925070000) / 100)))
    sm.setQRValue(DOJO_QUEST, "time", str(sm.getCurClockTimeSec()))