...
if selection == 4 and sm.hasQuest(preQs[selection]):
    ...
    sm.warpInstanceIn(maps[selection])
    sm.setInstanceTime(5*60)

if not sm.hasQuest(preQs[selection]) \
        and not sm.hasQuestCompleted(preQs[selection]) \
        and not sm.hasQuest(preQsDemon[selection]) \
        and not sm.hasQuestCompleted(preQsDemon[selection]):
