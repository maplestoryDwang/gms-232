# id 34922 (Repair Completed?!), field 402000600
sm.lockInGameUI(True, False)
sm.showFadeTransition(0, 1000, 3000)
sm.zoomCamera(0, 1000, 2147483647, 2147483647, 2147483647)
sm.moveCamera(True, 0, 0, 0)
sm.sendDelay(300)
sm.removeOverlapScreen(1000)
sm.zoomCamera(0, 2000, 0, 900, 150)
sm.forcedFlip(True)
sm.setSpeakerType(3)
sm.setParam(37)
sm.setColor(1)
sm.setInnerOverrideSpeakerTemplateID(3001500) # Ark
sm.sendNext("#face0#哈，重建工作终于完成了。维依说把东西放进仓库之后，再回去找他？")
sm.spawnNpc(3001511, 1000, 76)
sm.showNpcSpecialActionByTemplateId(3001511, "summon", 0)
sm.moveNpcByTemplateId(3001511, False, 300, 100)
sm.sendDelay(1000)
sm.sendNext("#face2#啊，那个孩子是之前的那个……叫马尔的孩子……")
sm.sendSay("#face2#他好像想到避难处外面去……太危险了。")
sm.zoomCamera(2000, 2000, 2000, 1200, 150)
sm.forcedMove(False, 300)
sm.sendDelay(2000)
sm.flipNpcByTemplateId(3001511, True)
sm.showNpcSpecialActionByTemplateId(3001511, "falldown", 0)
sm.sendDelay(3500)
sm.sendNext("#face2#从这里出去太危险了。快回到避难处去。")
sm.setInnerOverrideSpeakerTemplateID(3001511) # Mar
sm.sendSay("#face2#嘿嘿……")
sm.flipNpcByTemplateId(3001511, False)
sm.moveNpcByTemplateId(3001511, False, 150, 100)
sm.sendDelay(500)
sm.forcedMove(False, 300)
sm.setInnerOverrideSpeakerTemplateID(3001500) # Ark
sm.sendNext("#face4#拦不住他。为了以防万一，只能跟过去看看了。")
sm.zoomCamera(500, 2000, 500, 1300, 150)
sm.sendDelay(1000)
sm.completeQuestNoCheck(parentID)
sm.startQuest(11620)
sm.createQuestWithQRValue(15710, "lasttime=19/02/21/12/40")
sm.createQuestWithQRValue(parentID, "exp=1")
sm.createQuestWithQRValue(34995, "00=h1;01=h1;10=h0;02=h1;11=h0;12=h0;04=h1;13=h0;05=h0;14=h0;23=h0;06=h0;15=h0;24=h0;07=h0;16=h0;17=h0;09=h0")
sm.showFadeTransition(0, 1000, 3000)
sm.zoomCamera(0, 1000, 2147483647, 2147483647, 2147483647)
sm.moveCamera(True, 0, 0, 0)
sm.sendDelay(300)
sm.removeOverlapScreen(1000)
sm.lockInGameUI(False, True)
sm.warp(402000613)
