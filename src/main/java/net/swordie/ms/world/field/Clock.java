...
public class Clock {
    ...
    public Clock(ClockType clockType, Char chr, int seconds) {
        this.clockType = clockType;
        this.chr = chr;
        this.seconds = seconds;
        this.timeInMillis = (seconds*1000) + System.currentTimeMillis();
    ...
    public void createClockForChr() {
        switch (getClockType()) {
            case SecondsClock:
                chr.write(FieldPacket.clock(ClockPacket.secondsClock(seconds)));
                break;
            case StopWatch:
                chr.write(FieldPacket.clock(ClockPacket.stopWatch(seconds)));
                break;
            case TimerGauge:
                chr.write(FieldPacket.clock(ClockPacket.timerGauge(seconds * 1000, seconds * 1000)));
                break;
        }
        clockRemovalTimer = EventManager.addEvent(this::removeClockForChr, seconds, TimeUnit.SECONDS);
    }
    ...
    public void removeClockForChr() {
        ...
        EventManager.stopTimer(clockRemovalTimer);
    }
