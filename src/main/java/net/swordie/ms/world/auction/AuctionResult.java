...
public class AuctionResult implements Encodable {
    ...
    private int errorCode; // int because it's different per requestType
    ...
    public void encode(OutPacket outPacket) {
        // limit to 150 items, sorted by date
        if (items.size() > GameConstants.MAX_AUCTION_QUERY_RESULT_SIZE) {
            items = items.stream()
                         .sorted(Comparator.comparing(AuctionItem::getEndDate).reversed())
        ...
        if (errorCode != 0) {
            outPacket.encodeInt(errorCode);
        } else {
            outPacket.encodeInt(code.getVal()); // nResultCode
        }
        outPacket.encodeInt((int) auctionId); // nAuctionId
        switch (type) {
            case ListItem:
            ...
            case SearchMarketPlace:
                ...
                outPacket.encodeByte(bool3); // bYieldedTooManyItems
            ...
            case LoadSoldItems:
                ...
                for (AuctionItem auctionItem : items) {
                    ...
                    outPacket.encodeByte(auctionItem.getState() != AuctionState.Done);
                    if (auctionItem.getState() != AuctionState.Done) {
                        auctionItem.encode(outPacket);
                    } else {
                        outPacket.encodeByte(true); // bEncode
                        auctionItem.getItem().encode(outPacket);
                    }
                }
                break;
            case ViewAvgPrice:
    ...
    public static AuctionResult sellItem(boolean success, long auctionId) {
    ...
    public static AuctionResult requestSucceed(AuctionType auctionType, int auctionId) {
        AuctionResult ar = new AuctionResult(auctionType);
        ar.auctionId = auctionId;
        return ar;
    }

    public static AuctionResult loadSoldItems(Set<AuctionItem> items) {
        AuctionResult ar = new AuctionResult(AuctionType.LoadSoldItems);
        ar.items = items;
        return ar;
    }

    public static AuctionResult sellItemAdd(AuctionItem item) {
    ...
    public static AuctionResult soldItemAdd(AuctionItem item) {
    ...
    public static AuctionResult wishlistAdd(AuctionItem item) {
    ...
    public static AuctionResult error(AuctionType at, AuctionResultCode code) {
        ...
        // Etc/AuctionData.img/ErrorMsg/%d => requestType/errorCode
        ar.code = code;
        return ar;
    }
