...
public class Util {
    ...
    public static final Set<Life> EMPTY_SET = new HashSet<>();
    private static final char[] HEX_ARRAY = "0123456789ABCDEF".toCharArray();
    private static final Map<Class, Class> boxedToPrimClasses = new HashMap<>();
    private static final Map<Class, Class> primToBoxedClasses = new HashMap<>();
    private static final Pattern regexPattern = Pattern.compile("^\\$2[a-z]\\$.{56}$");
    private static final Pattern numberPattern = Pattern.compile("-?\\d+(\\.\\d+)?");
    private static final Pattern isDigitLetterStringPattern = Pattern.compile("[a-zA-Z0-9]+");
    private static final Pattern isValidStringPattern = Pattern.compile("[a-zA-Z0-9`~!@#$%^&*()_+-={}|\\\\;':\",./<>?\\[ ]*");
    private static final Pattern isIntegerPattern = Pattern.compile("^-?[0-9]+");
    ...
    static {
        ...
        primToBoxedClasses.put(boolean.class, Boolean.class);
        primToBoxedClasses.put(byte.class, Byte.class);
        primToBoxedClasses.put(short.class, Short.class);
        primToBoxedClasses.put(char.class, Character.class);
        primToBoxedClasses.put(int.class, Integer.class);
        primToBoxedClasses.put(long.class, Long.class);
        primToBoxedClasses.put(float.class, Float.class);
        primToBoxedClasses.put(double.class, Double.class);
    }
    ...
    /**
     * Returns a random number from <code>start</code> up to and including <code>end</code>. Creates a new Random class upon call.
     * If <code>start</code> is greater than <code>end</code>, <code>start</code> will be swapped with <code>end</code>.
     *
     * @param start the lower bound of the random number
     * @param end   the upper bound of the random number
     * @return A random number from <code>start</code> up to and including <code>end</code>
     */
    public static int getRandomInclUpperBound(int start, int end) {
        return getRandom(start, end + 1);
    }
    ...
    public static String readableByteArray(byte[] arr) {
        char[] hexChars = new char[arr.length * 3];
    ...
    public static Class<?> convertPrimitiveToBoxedClass(Class<?> clazz) {
        return boxedToPrimClasses.getOrDefault(clazz, clazz);
    }

    public static boolean isPrimitiveTypeForDB(Class clazz) {
        return boxedToPrimClasses.containsKey(clazz) || primToBoxedClasses.containsKey(clazz)
                || clazz == FileTime.class || clazz == String.class;
    }
    ...
    public static boolean isDigitLetterString(String str) {
        return str != null && isDigitLetterStringPattern.matcher(str).matches(); // maybe allow special characters?
    }
    ...
    public static boolean isValidString(String str) {
        return str != null && isValidStringPattern.matcher(str).matches();
    }
    ...
    public static boolean isInteger(String val) {
        if (val != null && isIntegerPattern.matcher(val).matches() && val.length() <= 10) {
    ...
    public static boolean isLong(String val) {
        return val != null && isIntegerPattern.matcher(val).matches() && val.length() <= 18;
    }
    ...
    public static String getStackFrame(String preString) {
        var sb = new StringBuilder(preString).append("\r\n");
    ...
    public static DropInfo getRandomDropInfoByChance(Set<DropInfo> dropInfos) {
        var total = dropInfos.stream().mapToInt(DropInfo::getChance).sum();
        var rng = getRandom(total);
        var currentChance = 0;

        for (var di : dropInfos) {
            currentChance += di.getChance();
    ...
    public static void xor(byte[] arr, byte[] other) {
        for (int i = 0; i < arr.length; i++) {
            arr[i] = (byte) (arr[i] ^ other[i]);
        }
    }

    public static int getTimeSince(long past, TimeUnit timeUnit) {
        var now = getCurrentTimeLong();
        var diff = Math.abs(now - past);

        return (int) timeUnit.convert(diff, TimeUnit.MILLISECONDS);
    }

    public static int getMinutesSince(long past) {
        return getTimeSince(past, TimeUnit.MINUTES);
    }

    public static int getSecondsSince(long past) {
        return getTimeSince(past, TimeUnit.SECONDS);
    }

    public static int getHoursSince(long past) {
        return getTimeSince(past, TimeUnit.HOURS);
    }

    /**
     * Returns the amount of seconds from now till {@param offsetDateTime}
     */
    public static long getSecondsTill(OffsetDateTime offsetDateTime) {
        var now = OffsetDateTime.now(ZoneId.systemDefault());
        return getSecondsTill(now, offsetDateTime);
    }

    /**
     * Returns the amount of seconds from {@param from} till {@param now}.
     */
    public static long getSecondsTill(OffsetDateTime from, OffsetDateTime to) {
        var nowEpochSec = from.toEpochSecond();
        var offsetEpochSec = to.toEpochSecond();
        return Math.abs(offsetEpochSec - nowEpochSec);
    }

    public static OffsetDateTime getStartNextDayAsOffsetDateTime() {
        OffsetDateTime now = OffsetDateTime.now();
        var zoneOffset = now.getOffset();

        // Get Next day from now.
        OffsetDateTime nextDay = now.plusDays(1);

        // Create OffsetDateTime object but init hour/min/sec as 0.
        return OffsetDateTime.of(nextDay.getYear(), nextDay.getMonthValue(), nextDay.getDayOfMonth(), 0, 0, 0, 0, zoneOffset);
    }

    /**
     * Gets last monday based on today
     * @return
     */
    public static FileTime getLastMonday() {
        LocalDateTime lastMonday = LocalDateTime.now().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
        LocalDateTime lastMondayStart = LocalDateTime.of(lastMonday.getYear(), lastMonday.getMonth(), lastMonday.getDayOfMonth(), 0, 0);
        return FileTime.fromDate(lastMondayStart);
    }

    /**
     * Gets next monday based on today
     *
     * @return
     */
    public static FileTime getNextMonday() {
        LocalDateTime nextMonday = LocalDateTime.now().with(TemporalAdjusters.next(DayOfWeek.MONDAY));
        LocalDateTime nextMondayStart = LocalDateTime.of(nextMonday.getYear(), nextMonday.getMonth(), nextMonday.getDayOfMonth(), 0, 0);
        return FileTime.fromDate(nextMondayStart);
    }

    /**
     * Translates the origin position towards {angle} for {length}
     *
     * @param origin
     * @param angle
     * @param length
     * @return
     */
    public static Position getPositionTranslation(Position origin, int angle, double length) {
        var originX = origin.getX();
        var originY = origin.getY();

        var angleR = Math.toRadians(angle);

        // sin is X here. as 6'o clock is 0 degrees for Obstacle atoms
        // cos is Y here. as 6'o clock is 0 degrees for Obstacle atoms
        var newX = originX + (length * Math.sin(angleR));
        var newY = originY - (length * Math.cos(angleR)); // Y is inverted

        return new Position((int) newX, (int) newY);
    }

    public static double getHypothenuseViaCAH(int angle, int adjacent) {
    ...
    public static int convertDegreeToDumbFuckingNexonsWayOfDoingDegreesForObstacleAtoms(int angle) {
        ...
        for (var t : tuples) {
            var cutOffAngle = t.getLeft();
            var angleTranslation = t.getRight();
