 ...
 */
public class FileTime implements Serializable, Comparable<FileTime> {
	...
	public boolean isAfter(FileTime fileTime) {
		return toLocalDateTime().isAfter(fileTime.toLocalDateTime());
	}
	...
	/**
	 * Returns the current date+time as YYYYMMDDHHMMSS format.
	 * @return created string
	 */
	public String toYYYYMMDDHHMMSS() {
		...
		LocalDateTime ldt = toLocalDateTime();
		return String.format("%04d%02d%02d%02d%02d%02d", ldt.getYear(), ldt.getMonthValue(), ldt.getDayOfMonth(),
				ldt.getHour(), ldt.getMinute(), ldt.getSecond());
	}

	public String toClientInYYYYMMDDHHMMSS() {
		if (isMaxTime()) {
			return "990101010101";
		} else if (isMinTime()) {
			return "700101010101";
		}
		LocalDateTime ldt = toLocalDateTime().plusHours(2);
		return String.format("%04d%02d%02d%02d%02d%02d", ldt.getYear(), ldt.getMonthValue(), ldt.getDayOfMonth(),
				ldt.getHour(), ldt.getMinute(), ldt.getSecond());
	}

	public static FileTime fromYYMMDDHHMMSS(String date) {
		LocalDateTime localDateTime = LocalDateTime.parse(date, DateTimeFormatter.ofPattern("yyMMddHHmmss"));
		return FileTime.fromDate(localDateTime);
	}
