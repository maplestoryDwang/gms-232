...
public class ScriptMemory {
    ...
    /**
     * Adds a new entry to the memory. Automatically increments the position. Will clear the memory if it has no prev button.
     * @param nsi the info to add
     */
    public void addMemoryInfo(NpcScriptInfo nsi) {
        if (!nsi.getMessageType().isPrevPossible()) {
            clear();
            return;
        }

        if (getMemory().size() > MAX_MEMORY) {
            getMemory().remove(0);
            setPosition(getPosition() - 1);
        }
