...
public class ScriptPacketParser {
    ...
    public void handlePortalScriptRequest(InPacket inPacket) {
        FieldInfo field = FieldData.getFieldInfoById(sm.getFieldId());
    ...
    public void handleSetField(InPacket inPacket) {
        ...
        for (int i = 4; i < len; i++) {
            ...
            if (fieldID >= 100000000 && FieldData.getFieldInfoById(fieldID) != null) {
                res = fieldID;
            }
        ...
        sm.createScript(ScriptMaker.MAY_OVERRIDE_FILES);
        FieldInfo field = FieldData.getFieldInfoById(res);
        if (field == null) {
    ...
    public void handleFieldEffect(InPacket inPacket) {
        ...
        switch (fet) {
            ...
            case SpineScreen:
                ...
                String str4 = inPacket.decodeString();

                inPacket.decodeByte(); // bIdk
                inPacket.decodeInt(); // nIdk1
                inPacket.decodeInt(); // nIdk2
                inPacket.decodeInt();
                inPacket.decodeInt();
