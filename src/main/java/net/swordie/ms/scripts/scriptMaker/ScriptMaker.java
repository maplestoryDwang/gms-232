...
public class ScriptMaker {
    ...
    public static void main(String[] args) {
        File file = new File("E:\\MapleDev\\sniffs\\sjonnie add this ty (sellas).msb");
        if (!file.exists()) {
        ...
        for (File f : files) {
            ...
            start = System.currentTimeMillis();
//            sm.makeShopsFromPackets(packets);
            sm.makeScriptsFromPackets(packets);
...
//                            createScript(ScriptMaker.MAY_OVERRIDE_FILES);
    ...
    public FieldInfo getField() {
        return FieldData.getFieldInfoById(getFieldId());
    }
    ...
    public void createScript(boolean mayOverride) {
        ...
        FieldInfo field = FieldData.getFieldInfoById(getFieldId());
        String name = "";
        int id = getId();
        switch (getType()) {
            case None:
