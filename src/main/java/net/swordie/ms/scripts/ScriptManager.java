...
public interface ScriptManager {
	 ...
	 * @param cts the cts of the skill that needs to be removed.
	 */
	void removeBuff(CharacterTemporaryStat cts);
	...
	/**
	 * enables the portal with specified name
	 * @param portalName
	 * 		Name of the target portal
	 * @param enabled
	 * 		whether or not the portal should be enabled
	 */
	void setPortalEnabled(String portalName, boolean enabled);
	...
	/**
	 * Drops the specified Items on the field at the given position (x, y).
	 *
	 * @param dropInfos
	 * 		The set of items that should be dropped.
	 * @param x
	 * 		The X-coordinate of the Position.
	 * @param y
	 * 		The Y-Coordinate of the Position.
	 * @param ownerId
	 * 		Player that owns the drop
	 */
	void dropItems(Set<Tuple<Integer, Integer>> dropInfos, int x, int y, int ownerId);
	 ...
	 *
	 * @param bossCooldown the boss to check the cooldown for. May be null.
	 *
	 * @return True if the entire party is in the same location/channel and the leader is the linked
	 * {@link Char}.
	 */
	boolean checkParty(BossCooldown bossCooldown);
	 ...
	 * @param bossCooldown the boss to check the cooldown for. May be null.
	 *
	 * @param reqLevel  Level required for all party members to be able to enter.
	 * @return True if the entire party is in the same location/channel and the leader is the linked
	 * {@link Char}.
	 */
	boolean checkParty(BossCooldown bossCooldown, int reqLevel);
	...
	/**
	 * Upgrades the secondary of the player. If there is no secondary, simply creates one.
	 * @param id
	 */
	 ...
	 */
	void createObstacleAtom(ObstacleAtomEnum oae, int key, int damage, int velocity, int angle, int amount, int proc);
	...
	void createDojoClock(int seconds);

	String getCurClockTime();

	long getCurClockTimeSec();
	...
	*/
	...
	/**
	 * Stops all active ScheduledFuture's that are tagged as field event in ScriptManagerImpl::isFieldEventMethodName
	 */
	 ...
	 *
	 * @return whether or not a nodestone was opened
	 */
	boolean openNodestone(int id);

	boolean levelArcaneSymbol(BodyPart symbolPart, int levelAmount);

	boolean levelAuthSymbol(BodyPart symbolPart, int levelAmount);
}
