...
public class MakingSkillRecipe {
    ...
    public int getRecommendedSkillLevel() {
        return recommendedSkillLevel;
    }

    public void setRecommendedSkillLevel(int recommandedSkillLevel) {
        this.recommendedSkillLevel = recommandedSkillLevel;
    }
    ...
    public boolean isAbleToBeUsedBy(Char chr) {
        ...
        if (getRecommendedSkillLevel() == SkillConstants.MAKING_SKILL_MASTER_LEVEL && makingSkillLevel < SkillConstants.MAKING_SKILL_MASTER_LEVEL) {
            chr.write(UserLocal.noticeMsg("You must be a Master Craftsman to craft this.", true));
            return false;
        }
        if (getRecommendedSkillLevel() == SkillConstants.MAKING_SKILL_MEISTER_LEVEL && makingSkillLevel < SkillConstants.MAKING_SKILL_MEISTER_LEVEL) {
        ...
        for (TargetElem target : getTarget()) {
            if (!chr.canHoldItem(target.itemID, target.count)) {// should handle count too
        ...
        if (makingSkillLevel >= SkillConstants.MAKING_SKILL_EXPERT_LEVEL) {
            ...
            if (!SkillConstants.isDecompositionRecipe(recipeID) && !SkillConstants.isSynthesizeRecipe(recipeID)) {
                ...
                if (success) {
                    ...
                    double recommandedSkillLevel = getRecommendedSkillLevel();
                    int flag = 3;
                    double rate = 1.0;
                    while (true) {
                        if ((flag & 1) != 0) {
                    ...
                    proficiency = Math.max(getRecommendedSkillLevel() - 7.0, 0.0) * rate;
                ...
                return Math.max(5, (int) proficiency);
            }
            return 0;
        }
        if (success) {
    ...
    private int getProficiency(int makingSkillLevel, boolean success) {
        ...
        int unk = getRecommendedSkillLevel() - makingSkillLevel;
    ...
    public static int getSuccessProb(int reqSkillID, int recommendedSkillLevel, int skillLevel) {
        if (reqSkillID == 92000000 || reqSkillID == 92010000) {
            return 100;
        }
        int rate = Math.max(0, 100 - (recommendedSkillLevel - skillLevel) * 20);
        if (rate >= 100) {
            rate = 100;
        }
        return rate;
