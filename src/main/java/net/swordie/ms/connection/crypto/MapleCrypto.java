...
public final class MapleCrypto {
    ...
    public MapleCrypto() {
        ...
        // ZLZ64.dll: sub_1800038F0, or search for C7 (mov [esp+XXh], 30h)
        //
        // OEP: XREF 4e e6 40 bb (security_init_cookie, xref -> make func)
        // Or: 83 EC 28 53 33 db, should be the E8 above that
        // ------------------------
        // v231
     ...
     *
     * @param data the input data to be put into stage for finalized encryption
     * or to be finally decryption.
     * @param key the input seed for this cryptography stage. This value is
    ...
    public byte[] crypt(byte[] data, byte[] key) {
        ...
        while (dataLen > 0) {
            byte[] multipliedBytes = multiplyBytes(key, 4, 4);
            if (dataLen < blockSize) {
                blockSize = dataLen;
            }
            for (int i = totalDecrypted; i < (totalDecrypted + blockSize); i++) {
                if ((i - totalDecrypted) % multipliedBytes.length == 0) {
                    cipher.encrypt(multipliedBytes);
                }
                data[i] ^= multipliedBytes[(i - totalDecrypted) % multipliedBytes.length];
            }
            totalDecrypted += blockSize;
            dataLen -= blockSize;
            blockSize = 1460;
        }
        return data;
     ...
     *
     * @param datalen the input packet length before adding the header.
     * @param iv the input sending seed before changing it.
     *
     * @return the header to be sent with this packet.
     */
    public static byte[] getHeader(int datalen, byte[] iv) {
        if (datalen >= Packet.MAX_SHORT_PACKET_SIZE) {
            datalen = Packet.MAX_SHORT_PACKET_SIZE;
        }

        int a = (((iv[3] << 8) & 0xFF00) | (iv[2] & 0xFF));
        a ^= -(ServerConstants.VERSION + 1);
        int xorLen = a ^ datalen;

        byte[] ret = new byte[4];
        ret[0] = (byte) (a & 0xFF);
        ret[1] = (byte) ((a >>> 8) & 0xFF);
        ret[2] = (byte) (xorLen & 0xFF);
        ret[3] = (byte) ((xorLen >>> 8) & 0xFF);
        return ret;
    }
