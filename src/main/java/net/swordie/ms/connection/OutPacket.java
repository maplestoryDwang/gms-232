...
public class OutPacket extends Packet {
     ...
     * @param l The long to encode.
     */
    public void encodeLongBE(long l) {
        encodeIntBE((int) ((l >>> 32L) & 0xFFFFFFFFL));
        encodeIntBE((int) (l & 0xFFFFFFFFL));
    }

    public void encodeDouble(double d) {
        encodeLong(Double.doubleToRawLongBits(d));
    }
    ...
    public void encodeRectShort(Rect rect) {
        encodeShort(rect.getLeft());
        encodeShort(rect.getTop());
        encodeShort(rect.getRight());
        encodeShort(rect.getBottom());
    }
    ...
    public int encodeEncryptedBuffer(byte[] buffer) {
        int randomKey = Util.getRandom(Integer.MAX_VALUE - 1);

        encodeInt(randomKey);
        encodeInt(buffer.length);
        encodeEncryptedBuffer(randomKey, buffer);

        return randomKey;
    }

    public void encodeEncryptedBuffer(int key, byte[] buffer) {
        ...
        for (; i < size; i++) {
            int curByte = buffer[i] & 0xFF;
            curByte ^= 0xAB;
            curByte -= key >>> 7;
            curByte -= 55;
            curByte ^= key >>> 1;

            resBuf[i] = (byte) (curByte & 0xFF);
        }
