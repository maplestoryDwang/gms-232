...
public abstract class WzObject<E extends WzObject, T extends WzObject<?, ?>> implements Comparable<E>, Iterable<T> {
	...
	public T getChild(String name, Object defaultValue) {
		T child = getChild(name);
		if (child == null) {
			child = (T) new WzEmpty<>("empty", defaultValue);
		}
		return child;
	}

	public T getChild(Object defaultValue, String... possibleNames) {
		...
		return (T) new WzEmpty<>("empty", defaultValue);
	}
    ...
    public void clear() {
		// oof
		if (this instanceof WzImage) {
			((WzImage) this).unparse();
		} else {
			getChildren().clear();
		}
    }

	protected WzProperty getPropertyValue() {
		if (this instanceof WzProperty) {
			return ((WzProperty) this);
		}
		return null;
	}
}
