...
public class HitInfo {
    ...
    public MobSkill mobAttackSkill;
    public int fixDamR;
    ...
    public void setMobAttackSkill(Field field) {
        if (mobID != 0 && (hpDamage > 0 || mpDamage > 0)) {
            Life life = field.getLifeByObjectID(mobID);
            if (life instanceof Mob) {
                var mob = (Mob) life;
                var mobInfo = mob.getMobInfo();
                MobSkill ms = null;
                if (attackIdx == -1 && mobInfo.getBodyDiseaseLevel() > 0) {
                    MobSkillInfo msi = SkillData.getMobSkillInfoByIdAndLevel(mobInfo.getBodyDisease(), mobInfo.getBodyDiseaseLevel());
                    if (msi != null) {
                        ms = new MobSkill();
                        ms.setDisease(mobInfo.getBodyDisease());
                        ms.setLevel(mobInfo.getBodyDiseaseLevel());
                    }
                } else {
                    ms = mob.getAttackById(attackIdx);
                }
                if (ms != null) {
                    fixDamR = ms.getFixDamR();
                }
                mobAttackSkill = ms;
            }
        }
