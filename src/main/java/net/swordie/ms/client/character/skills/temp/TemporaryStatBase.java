...
public class TemporaryStatBase {
    ...
    public boolean hasExpired(long time) {
        ...
        if (isDynamicTermSet() || (getExpireTerm() != 0)) {
            result = getExpireTerm() < FileTime.fromEpochMillis(time).toLong() - getLastUpdated().toLong();
        }
        return result;
    }
    ...
    public void encode(CharacterTemporaryStat cts, OutPacket outPacket) {
        ...
        outPacket.encodeInt(cts.equals(CharacterTemporaryStat.RideVehicleExpire) ? 0 : getExpireTerm());
        if (isDynamicTermSet() || cts.equals(CharacterTemporaryStat.RideVehicleExpire)) {
