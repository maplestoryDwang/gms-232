...
public class ExpIncreaseInfo {
    ...
    public void encode(OutPacket outPacket) {
        ...
        if ((mask & CantEncoding.getVal()) != 0) {
            outPacket.encodeInt(3);
        }
        if ((mask & FieldBonusExp.getVal()) != 0) {
            outPacket.encodeInt(4);
        }
        if ((mask & BigBangAttackBonusExp.getVal()) != 0) {
            outPacket.encodeInt(5);
        }
        if ((mask & WorldGaugeBonusExp.getVal()) != 0) {
            outPacket.encodeInt(6);
        }
        if ((mask & TestEverything.getVal()) != 0) {
            outPacket.encodeInt(7);
        }
        if ((mask & GuildKnowhowBonusExp.getVal()) != 0) {
        ...
        if ((mask & Empty.getVal()) != 0) {
            outPacket.encodeInt(12);
        }
        if ((mask & SetItemBonusExp.getVal()) != 0) {
            outPacket.encodeInt(13);
        }
        if ((mask & SetItemPartyBonusExp.getVal()) != 0) {
            outPacket.encodeInt(14);
        }
        if ((mask & AbsoluteTasteBonusExp.getVal()) != 0) {
            outPacket.encodeInt(15);
        }
        if ((mask & SnowflakeRingBonusExp.getVal()) != 0) {
