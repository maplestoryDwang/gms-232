...
public class Life {
    ...
    public MobGen createMobGenFromLife() {
        ...
        if (getLifeType().equalsIgnoreCase("m")) {
            ...
            MobInfo mob = MobData.getMobInfoById(getTemplateId());
    ...
    public String toString() {
        return String.format("ObjectID: %d, TemplateID: %d, Type: %s", getObjectId(), getTemplateId(), getClass().getSimpleName());
    }
    ...
    public Rect getRectAround(Rect rect) {
        return getPosition().getRectAround(rect);
    }
    ...
    public void reenterFieldAfterWarp(Char chr) {
        chr.getField().spawnLife(this, null);
    }
    ...
    public <T extends Life> T castTo(Class<T> clazz) {
        try {
