function enter() {
    ...
    if (cm.getNumberFromQuestInfo(63485, a) == 0) {
        ...
        cm.onSetMapTagedObjectVisible(1, "bton_s_b3c_02", 0);
        cm.fieldEffect_PlayFieldSound("Sound/SoundEff.img/blackHeaven/door_open", 100);
        cm.getMap().onSyncDynamicFoothold(["door_s_b3c_03"], [0], [2], [2769], [-174]);
        cm.dynamicObjMove(2769, -174, 2769, -712, 2769, -174, 1, 0, -1, "door_s_b3c_03")
    }
}
var status = -1;
var selectionLog = [];
...
function action(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status === a++) {
            cm.sendOk("这个传送点的脚本尚未修复。\r\n脚本位于： 脚本/传送点/door_s_b3c_03.js ", 0)
        } else {
            cm.dispose()
        }
    }
