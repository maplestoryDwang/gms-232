function enter() {
    ...
    if (cm.getNumberFromQuestInfo(63485, a) == 0) {
        ...
        cm.onSetMapTagedObjectVisible(1, "bton_s_b3c_00", 0);
        cm.fieldEffect_PlayFieldSound("Sound/SoundEff.img/blackHeaven/door_open", 100);
        cm.getMap().onSyncDynamicFoothold(["door_s_b3c_00"], [0], [2], [-2526], [-174]);
        cm.dynamicObjMove(-2526, -174, -2526, -712, -2526, -174, 1, 0, -1, "door_s_b3c_00")
    }
}
var status = -1;
var selectionLog = [];
...
function action(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status === a++) {
            cm.sendOk("这个传送点的脚本尚未修复。\r\n脚本位于： 脚本/传送点/door_s_b3c_00.js ", 0)
        } else {
            cm.dispose()
        }
    }
