...
function action(f, E, e) {
  ...
  if (status <= V++) {
    ...
    if (status === V++) {
      if (!cm.isQuestActive(39910) || cm.getNumberFromQuestInfo(39910, 'dir') > 0) {
        cm.dispose();
        return;
      }
      cm.setStringForQuestInfo(39900, '08', 'h1');
      cm.setStringForQuestInfo(39900, '09', 'h1');
      cm.setStringForQuestInfo(39900, '10', 'h1');
      cm.setStringForQuestInfo(39900, '23', 'h1');
        ...
        if (status === V++) {
          ...
          if (status === V++) {
            ...
            if (status === V++) {
              ...
              cm.effect_Text(["#fn黑体##fs18#当晚，翼人军营地附近。"], [100, 1000, 6, -50, -50, 1, 4, 0, 0, 0, 0]);
            } else {
              if (status === V++) {
