...
function action(f, E, e) {
  ...
  if (status <= V++) {
    ...
    if (status === V++) {
      ...
      cm.npc_ChangeController(9400202, "oid=23113323", -850, 2100, 60, -900, -800, 1, false, 0, false);
      cm.npc_SetSpecialAction("oid=23113323", "summon", 0, 0);
      cm.inGameDirectionEvent_ForcedFlip(1);
      cm.inGameDirectionEvent_MoveAction(4);
      cm.emotion(18, 15000);
      cm.inGameDirectionEvent_PushScaleInfo(0, 2000, 0, -890, 2200);
      cm.curNodeEventEnd(true);
    } else {
      if (status === V++) {
          ...
          if (status === V++) {
            ...
            if (status === V++) {
              ...
              if (status === V++) {
                ...
                if (status === V++) {
                  cm.sendNormalTalk_Bottom("哎呦喂，我还担心你要是真死了，我该怎么给你写墓碑……“这里沉睡着在魔法密林的树枝上一脚踩空摔死的勇士#h0#”这么写不是太凄惨了吗？", 37, 9400202, true, true);
                } else {
                  if (status === V++) {
                    cm.inGameDirectionEvent_ForcedFlip(1);
                    cm.inGameDirectionEvent_AskAnswerTime(500);
                      ...
                      if (status === V++) {
                        ...
                        if (status === V++) {
                          ...
                          if (status === V++) {
                            ...
                            if (status === V++) {
                              cm.sendNormalTalk_Bottom("#b(天啊……在异逝界发生的事情全部都是做梦吗？还是说我现在正梦见自己回到了冒险岛世界？)", 57, 0, true, true);
                            } else if (status === V++) {
                              cm.setStandAloneMode(false);
                              cm.dispose();
                              cm.warp(parseInt(cm.getInfoQuest(867113690)), 0, false);
                              cm.setInGameDirectionMode(false, true, false);
                              cm.npc_LeaveField("oid=23113323");
                              cm.npc_LeaveField("oid=23113323");
                            }
                          }
                        }
