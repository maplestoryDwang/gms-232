...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/1445.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                cm.askYesNo("和真正的海盗凯琳的战斗，使你成为了真正的海盗。感觉到了吗？现在剩下的就是转职了。做好成为更强的海盗——斗士的准备了吗？", 0, 2020013)
            } else {
                if (status === a++) {
                    cm.forceCompleteQuest(29902);
                    cm.forceStartQuest(1445, "");
                    cm.forceCompleteQuest(1445);
                    cm.updateInfoQuest(1073, "500=10;510=201;511=201");
                    cm.forceStartQuest(17905, "1");
                    cm.forceCompleteQuest(17905);
                    cm.playerMessage(5, "冒险之书任务已完成。");
                    cm.changeJob(cm.getJob() + 1);
                    cm.sendNormalTalk("现在起你是#b斗士#k啦。#b拳头和拳甲#k的大师，真正的斗士，去尽情施展你的力量吧。", 0, 2020013, false, false)
                } else {
                    if (status === a++) {
