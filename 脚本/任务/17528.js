  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      ...
      if (status === v++) {
        ...
        if (status === v++) {
          cm.forceStartQuest(17528, '');
          cm.sendNormalTalk("如果能先让他的头部瘫痪，就能向着制服贝勒德迈出一大步。", 0, 9390120, false, false);
        } else if (status === v++) {
          cm.dispose();
        }
      }
...
function end(g, w, a) {
  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      cm.sendNormalTalk("祝贺你。你成功地让贝勒德的头部瘫痪了。没想到你拥有这么强的力量，看来应该用不了多久就能让贝勒德恢复原来状态。接下去该进行下一阶段的工作了。", 0, 9390120, false, true);
    } else {
      if (status === v++) {
