...
function start(g, w, a) {
  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      ...
      if (status === v++) {
        cm.sendNormalTalk_Bottom("#face1#如果你今天能找到#i4036762:# #t4036762:#  30个就好了。\r\n消灭#b等级范围怪物#k后，应该就能获得建筑材料。", 36, H, true, true, 1);
      } else {
        if (status === v++) {
...
function end(g, w, a) {
  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      ...
      if (status === v++) {
        cm.sendNormalTalk_Bottom("#face1#多亏了#h0#，海滨露台小屋可使用的空间比以前宽敞很多呢。 ", 36, H, true, true, 1);
      } else {
        if (status === v++) {
          cm.sendNormalTalk_Bottom("#face1#竣工之后，我想立马去看看！#h0#也很好奇吧？我们到时候一起去看看吧！", 36, H, true, true, 1);
        } else if (status === v++) {
          cm.forceCompleteQuest();
          cm.updateInfoQuest(501430, "progress=2;today=1");
          cm.gainItem(4036762, -30);
          cm.playerMessage(-1, "可以使用海滨露台小屋扩建型图纸啦！");
          cm.我的小屋_解锁房屋户型(1, "海滨露台小屋", 23, 25);
          cm.fieldEffect_PlayFieldSound("Sound/Game.img/QuestClear", 100);
          cm.effect_OnUserEff("Effect/BasicEff.img/QuestClear");
          cm.dispose();
        }
