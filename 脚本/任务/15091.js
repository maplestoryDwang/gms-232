      ...
      if (status === h++) {
        ...
        if (status === h++) {
          ...
          if (status === h++) {
            ...
            if (status === h++) {
              cm.forceStartQuest();
              cm.sendNormalTalk("请你利用自己强大的力量，消灭#b10次#k#b进阶半半#k。", 0, 1064002, false, true, 0);
            } else {
              if (status === h++) {
                cm.sendNormalTalk("给你#i1142902:# #b#t1142902##k作为礼物吧。", 0, 1064002, true, true, 0);
              } else {
                if (status === h++) {
...
function end(K, s, F) {
  ...
  if (status <= h++) {
    ...
    cm.sendNormalTalk("你真是太了不起了！请收下这份礼物！#b#i1142902##z1142902#", 0, 1064002, true, false, 0);
    cm.forceCompleteQuest();
    cm.gainItem(1142902, 1);
    cm.forceCompleteQuest(15097);
    cm.dispose();
  }
}