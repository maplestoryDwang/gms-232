  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      ...
      if (status === v++) {
        ...
        if (status === v++) {
          cm.sendNormalTalk("如果能让他的左肩瘫痪，他就会变弱很多。肩膀瘫痪之后，保护他的灵气就会变弱，体力就会减半。和头部战斗的时候各种状态异常效果的发生次数也会大大减少。", 0, 9390120, true, true);
        } else {
          if (status === v++) {
            cm.askYesNo("怎么样？你想试试看吗？", 0, 9390120);
          } else {
            if (status === v++) {
              cm.forceStartQuest(17526, '');
              cm.sendNormalTalk("如果能先让他的左肩瘫痪，就能向着制服贝勒德迈出一大步。", 0, 9390120, false, false);
            } else if (status === v++) {
              cm.dispose();
            }
          }
...
function end(g, w, a) {
  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      cm.sendNormalTalk("祝贺你。你成功地让贝勒德的左肩瘫痪了。没想到你拥有这么强的力量，看来应该用不了多久就能让贝勒德恢复原来状态。接下去该进行下一阶段的工作了。", 0, 9390120, false, true);
    } else {
      if (status === v++) {
