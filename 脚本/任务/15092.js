      ...
      if (status === v++) {
        ...
        if (status === v++) {
          ...
          if (status === v++) {
            ...
            if (status === v++) {
              cm.forceStartQuest();
              cm.sendNormalTalk("请你利用自己强大的力量，消灭#b10次#k#b进阶皮埃尔#k。", 0, 1064002, false, true, 0);
            } else {
              if (status === v++) {
                cm.sendNormalTalk("给你#i1142903:# #b#t1142903##k作为礼物吧。", 0, 1064002, true, true, 0);
              } else {
                if (status === v++) {
...
function end(e, c, T) {
  ...
  if (status <= v++) {
    ...
    cm.sendNormalTalk("你真是太了不起了！请收下这份礼物！#b#i1142903##z1142903#", 0, 1064002, true, false, 0);
    cm.forceCompleteQuest();
    cm.gainItem(1142903, 1);
