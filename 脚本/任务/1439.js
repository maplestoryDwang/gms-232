...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/1439.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                cm.askYesNo("和真正的弓箭手赫丽娜的战斗，会把你引上真正的弓箭手之路……你感觉到自己成长了吗？现在剩下的事情就是转职了。做好成为更强的弓箭手——射手的准备了吗？", 0, 2020010)
            } else {
                if (status === a++) {
                    cm.forceCompleteQuest(29902);
                    cm.forceStartQuest(1439, "");
                    cm.forceCompleteQuest(1439);
                    cm.updateInfoQuest(1073, "300=10;310=201;311=201");
                    cm.forceStartQuest(17905, "1");
                    cm.forceCompleteQuest(17905);
                    cm.playerMessage(5, "冒险之书任务已完成。");
                    cm.changeJob(cm.getJob() + 1);
                    cm.sendNormalTalk("现在起你是#b盗贼#k啦。#b弓#k的大师，真正的盗贼，去尽情施展你的力量吧。", 0, 2020010, false, false)
                } else {
                    if (status === a++) {
