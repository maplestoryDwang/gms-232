...
function start(g, w, a) {
    ...
    if (Math.floor(O / 100) == 162) {
        if (status <= v++)
            ...
            cm.sendNormalTalk('这场和堕落超越者#r黑魔法师#k之间关于世界兴亡的大战终于落幕了。', 4, 3004150, true, true);
        else if (status === v++)
            cm.askYesNo('既然如此，你难道就不好奇黑魔法师消亡之后的故事会怎样吗？\r\n\r\n#r（※接受后前往活动地图。）\r\n（※推荐打开背景音乐进行。)', 4, 3004150);
        else if (status === v++)
            cm.sendNormalTalk('来，那我们就一起来看看#r决战那日之后#k吧？', 4, 3004150, false, true);
        else if (status === v++) {
...
function stage0(g, w, a) {
    ...
    if (status <= v++)
        ...
        cm.updateInfoQuest(39701, 'NpcSpeech=30042021');
        cm.dispose();
    }
}
function stage1(g, w, a) {
    ...
    if (status <= v++)
        ...
        cm.updateInfoQuest(39701, 'NpcSpeech=30042021/30042032');
        cm.dispose();
    }
}
function end(g, w, a) {
    ...
    if (status <= v++)
        ...
        cm.updateInfoQuest(39701, 'NpcSpeech=30042021/30042032/30042033');
