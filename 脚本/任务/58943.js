...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/58943.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            cm.forceCompleteQuest(58943);
            cm.sendNormalTalk("没错，我看到的坛子就是这个。", 4, 9130107, false, true);
            cm.gainItem(4034159, -1);
            cm.gainItem(4034160, -1);
            cm.gainItem(4034161, -1)
        } else {
            if (status === a++) {
