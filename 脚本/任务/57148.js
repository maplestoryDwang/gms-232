...
function start(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                cm.askAcceptDecline_Bottom("#face0#之所以你所说的这些人会来到武陵，应该也是为了这个目的吧。这次的问题似乎出现在蟠桃果林。如果我没有猜错，他们应该是想要进入果林深处的森林，因为那里是武陵中灵气最为鼎盛的地区。你要不要进去调查看看？", 37, 2091001)
            } else {
                if (status === a++) {
                    ...
                    if (status === a++) {
                        ...
                        cm.dispose();
                        cm.forceStartQuest(57148, "");
                        cm.warp(250010503, 1, false)
                    }
                }
            }
        }
...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/57148.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
