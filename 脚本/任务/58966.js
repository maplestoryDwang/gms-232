...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/58966.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            cm.sendNormalTalk("嗯，看来浓姬的品味真是奇怪啊。", 16, 0, false, true)
        } else {
            if (status === a++) {
                cm.sendNormalTalk("那个少年会在建筑里吗？其实那个少年可能不重要……总之先进去看看吧。", 16, 0, true, true)
            } else {
                if (status === a++) {
                    cm.forceCompleteQuest(58966);
                    cm.dispose()
                }
            }
        }
