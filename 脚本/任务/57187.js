...
function start(g, w, a) {
  ...
  if (status <= v++) {
    ...
    if (status == v++) {
      cm.sendNormalTalk_Bottom("#face0#哦？我还是第一次见到穿成这样子的人呢。你看起来似乎并不是从“岛”那边过来的……难道是来自“大陆”的吗？", 37, 9110002, false, true, 1);
    } else {
      if (status === v++) {
        ...
        if (status === v++) {
          ...
          if (status === v++) {
            cm.sendNormalTalk_Bottom("#face0#让我想想……我应该是第一次见到你这种穿着的人呢。不过原来你正在找人啊。但你说的这个人并没有来到古代神社呢。", 37, 9110002, true, true, 1);
          } else {
            if (status === v++) {
              ...
              if (status === v++) {
                ...
                if (status === v++) {
                  ...
                  if (status === v++) {
                    ...
                    cm.forceStartQuest(57187, '');
                    cm.npc_LeaveField("oid=103676");
                    cm.teachSkill(41000003, 10, 0);
                    cm.teachSkill(41001000, 20, 0);
