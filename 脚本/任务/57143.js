                                                ...
                                                if (status === a++) {
                                                    ...
                                                    if (status === a++) {
                                                        ...
                                                        if (status === a++) {
                                                            ...
                                                            if (status === a++) {
                                                                cm.askAcceptDecline_Bottom("#face0#我明白了。既然这样，就没有什么其他好问的了。剑斗，你已经好久没见到姬儿了，要不要和她好好聊聊呢？", 37, 9130010)
                                                            } else {
                                                                if (status === a++) {
                                                                    cm.forceStartQuest(57143, "");
                                                                    cm.dispose()
                                                                }
                                                            }
                                                        }
...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/57143.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            cm.askYesNo_Bottom("#face0#好了，剑斗你最近过得如何？快来陪我说说话吧。", 37, 9130021)
        } else {
            if (status === a++) {
                cm.gainExp(19112);
                cm.sendNormalTalk_Bottom("（回头想想，我和姬儿早在15年前就已经认识了。）", 57, 0, false, true)
            } else {
                if (status === a++) {
                    cm.sendNormalTalk_Bottom("（我家代代都是侍奉松山家的忠臣。父亲是松山家的元老，而哥哥和我也在负责处理松山家的公务。）", 57, 0, true, true)
                } else {
                    if (status === a++) {
                        cm.sendNormalTalk_Bottom("（不同于擅长处理政务的哥哥，我比较喜欢学习剑术。从小进行武士修行的时候，我学会了晓月流剑术。现在，这个剑术已经被我学得融会贯通，这也是我最引以为傲的事情。）", 57, 0, true, true)
                    } else {
                        if (status === a++) {
                            cm.sendNormalTalk_Bottom("（结束武士修行回到城里后，我见到了久别的姬儿……那时的她实在是太美了，美得让我不知该如何表述。笨嘴拙舌的我实在无法形容出她的美丽。可以说，她简直是集可爱与清纯于一身。）", 57, 0, true, true)
                        } else {
                            if (status === a++) {
                                cm.sendNormalTalk_Bottom("（但是，随着光秀的袭击，信包大人和以父亲、哥哥为首的大部分家臣全部惨遭杀害。唯一值得庆幸的是，姬儿逃过一劫，落在了织田军的手里。听说这个消息后，我立刻投奔了信玄大人，并一直在等待着能将姬儿救出来的机会。)", 57, 0, true, true)
                            } else {
                                if (status === a++) {
                                    cm.sendNormalTalk_Bottom("（之后，本能寺攻略战和这次的异常变化接踵而来……在经历了各种各样的事情之后，我总算能再次与姬儿团聚……）", 57, 0, true, true)
                                } else {
                                    if (status === a++) {
                                        cm.sendNormalTalk_Bottom("（剩下要做的，就是打倒织田军和信长，返回我们原本的世界——日本重建家门……这场不可思议的旅行应该也快要结束了吧。）", 57, 0, true, true)
                                    } else {
                                        if (status === a++) {
                                            cm.forceCompleteQuest(57143);
                                            cm.dispose()
                                        }
                                    }
                                }
