    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                ...
                if (status === a++) {
                    cm.forceCompleteQuest(63042);
                    cm.addNumberForQuestInfo(63042, "dayNR", 1);
                    cm.setNumberForQuestInfo(63042, "start", 2);
                    cm.setStringForQuestInfo(63042, "lastDate", cm.getNowTimeString(3));
                    if (cm.rand(0, 1) == 0) {
                        cm.forceStartQuest(63043, "");
                        cm.sendNormalTalk("好。那你去收集#t4034994:#给我填肚子吧。\r\n我看20个应该差不多够吃饱了。", 4, 9400215, false, false)
                    } else {
                        cm.forceStartQuest(63044, "");
...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/63042.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
