...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/1442.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                cm.askYesNo("和真正的飞侠达克鲁的战斗，让你变成了真正的飞侠……虽然你自己还没感觉到。现在剩下的事情就只有转职了。做好成为更强的飞侠——无影人的准备了吗？", 0, 2020011)
            } else {
                if (status === a++) {
                    cm.forceCompleteQuest(29902);
                    cm.forceStartQuest(1442, "");
                    cm.forceCompleteQuest(1442);
                    cm.updateInfoQuest(1073, "400=10;410=202;411=203");
                    cm.forceStartQuest(17905, "1");
                    cm.forceCompleteQuest(17905);
                    cm.playerMessage(5, "冒险之书任务已完成。");
                    cm.changeJob(cm.getJob() + 1);
                    cm.sendNormalTalk("现在起你是#b无影人#k啦。#b拳套和飞镖#k的大师，真正的无影人，去尽情施展你的力量吧。", 0, 2020011, false, false);
