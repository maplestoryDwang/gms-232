...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/1436.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                ...
                if (status === a++) {
                    ...
                    cm.forceStartQuest(1436, "");
                    cm.forceCompleteQuest(1436);
                    cm.updateInfoQuest(1073, "200=10;220=202;221=203");
                    cm.forceStartQuest(17905, "1");
                    cm.forceCompleteQuest(17905);
                    cm.playerMessage(5, "冒险之书任务已完成。");
                    cm.changeJob(221);
                    cm.sendNormalTalk("现在你是#b巫师#k啦。#b雷和冰#k的大师，真正的巫师，去尽情施展你的力量吧。", 0, 2020009, false, false)
                } else {
                    if (status === a++) {
