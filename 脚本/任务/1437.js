...
function stage0(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            var e = cm.getQuest();
            cm.askYesNo("这个任务的依次对话脚本还没有修复哦。它的脚本位于： #b 脚本/任务/1437.js#k\r\n\r\n如果你有兴趣，欢迎一起来修复！\r\n\r\n那么现在，你要立刻开始这个任务吗？")
        } else {
            if (status == a++) {
                cm.forceStartQuest();
                cm.dispose()
...
function end(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            ...
            if (status === a++) {
                cm.askYesNo("通过和真正的魔法师汉斯的战斗，你也获得了成为真正魔法师的资格……剩下的事情就是转职了。你想成为更强的魔法师——祭司吗？", 0, 2020009)
            } else {
                if (status === a++) {
                    cm.forceCompleteQuest(29902);
                    cm.forceStartQuest(1437, "");
                    cm.forceCompleteQuest(1437);
                    cm.updateInfoQuest(1073, "200=10;230=211;231=211");
                    cm.forceStartQuest(17905, "1");
                    cm.forceCompleteQuest(17905);
                    cm.playerMessage(5, "冒险之书任务已完成。");
                    cm.changeJob(cm.getJob() + 1);
                    cm.sendNormalTalk("现在你就是#b祭司#k了。希望你能作为#b治愈与神圣#k的大师、作为真正的祭司，尽情施展自己的力量。", 0, 2020009, false, false)
                } else {
                    if (status === a++) {
