...
function start(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status == a++) {
            cm.sendNormalTalk("(之前见过的信又到了我的手里。)有新任务吗？", 16, 9073020, false, true)
        } else {
            if (status === a++) {
                cm.sendNormalTalk("至#r#m230040410##k、#b#p9073023##k。", 0, 9073020, true, true)
            } else {
                if (status === a++) {
                    cm.askAcceptDecline("看来发生了比上次更紧急的任务，怎么办？", 2, 9073020)
                } else {
                    if (status === a++) {
                        cm.forceStartQuest(1664, "");
                        cm.sendNormalTalk("还好放得进移动券。赶紧用它前往#p9073023#吧。", 17, 9073020, false, true);
                        cm.gainItem(2030027, 1)
                    } else {
                        if (status === a++) {
                            cm.fieldEffect_ScreenMsg("crossHunter/chapter/start4");
                            cm.dispose()
                        }
                    }
                }
