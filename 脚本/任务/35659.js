...
function start(mode, type, selection) {
	...
	if (status <= i++) {
		...
		cm.askYesNo("请击败黑魔法师的残影，找到1个#i4036458# #b#z4036458#，#k并且击败军团长 #r普通阿卡伊勒#k的残影。你能做到吗？", 4, 9063152);
	} else if (status == i++) {
		cm.forceStartQuest();
		var jobId = cm.getJob();
		if (jobId == '10112') {
			cm.sendOk("你应该已经知道了，黑暗痕迹只有在特殊条件下才能被刻印上去。请在满足以下条件的同时击败#r普通阿卡伊勒。#k\r\n\r\n" +
				"#e#r<任务完成条件>#k#n\r\n" +
				" #b- 独自攻破\r\n" +
				" - 只装备封印的创世武器和辅助武器\r\n" +
				" （神之子不装备封印的武器。)\r\n" +
			...
			cm.sendOk("你应该已经知道了，黑暗痕迹只有在特殊条件下才能被刻印上去。请在满足以下条件的同时击败#r普通阿卡伊勒。\r\n\r\n" +
				"#e#r<任务完成条件>#k#n\r\n" +
				" #b- 独自攻破\r\n" +
				" - 只装备封印的创世武器和辅助武器\r\n" +
				" - 最终伤害降低75%\r\n" +
...
function end(mode, type, selection) {
	...
	if (status <= i++) {
		...
		cm.sendNormalTalk_Bottom("#face0#你回来了啊，#b#h0##k。如果我的调查没有问题……\r\n看来武器中已经可以刻印上了黑暗痕迹。", 36, 9063153, false, true);
	} else if (status === i++) {
		cm.inGameDirectionEvent_AskAnswerTime(1700);
	} else if (status === i++) {
		cm.sendNormalTalk_Bottom("#face0#……那就开始吧。\r\n提取黑魔法师之片鳞的魔力传送到武器中。", 36, 9063153, false, true);
	} else if (status === i++) {
