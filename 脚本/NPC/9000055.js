  ...
  if (f != 1) {
    ...
    if (status == 0) {
      ...
      if (status == 1) {
        ...
        if (status == 2) {
          if (U == 0) {
            cm.askNumber("你想给我多少#b温暖的阳光#k？我会好好照顾枫树的。", cm.getItemQuantity(4001246), 0, cm.getItemQuantity(4001246));
          } else {
            cm.sendOk("枫树的成长状况：\r\n已捐献#r" + cm.getSunshines() + "#k个，需要5000个\r\n如果你有#r温暖的阳光#k，记得拿给我。");
            cm.dispose();
          }
        } else if (status == 3) {
          if (U < 0 || U > cm.getItemQuantity(4001246)) {
            U = cm.getItemQuantity(4001246);
          }
          if (U == 0) {
            cm.sendOk("请带来一些#b温暖的阳光#k");
          } else {
            cm.addSunshines(U);
            cm.gainItem(4001246, -U);
