...
function action(f, W, U) {
  if (status >= 27 && f == 0) {
    ...
    if ((status == 1 || status == 2 || status == 26) && f == 0 || (status == 6 || status == 9 || status == 12 || status == 15 || status == 18 || status == 21) && f == 1) {
      cm.dispose();
      return;
    }
  }
      ...
      if (status == 2) {
        if (U == 0) {
          ...
          if (U == 1) {
            status = 26;
            if (cm.getJob() == 0) {
              var V = "新手玩家可以获得#b90%#k的折扣。请选择目的地。#b";
              for (var w = 0; w < maps.length; w++) {
                V += "\r\n#L" + w + "##m" + maps[w] + "# (" + costBeginner[w] + " mesos)#l";
              }
            } else {
              var V = "请选择你的目的地吧。按照目的地不同，车费也有所不同。#b";
              for (var w = 0; w < maps.length; w++) {
                                                    ...
                                                    if (status == 25) {
                                                      ...
                                                      if (status == 26) {
                                                        ...
                                                        if (status == 27) {
                                                          ...
                                                          cm.askYesNo("I guess you don't need to be here. Do you really want to move to #b#m" + maps[U] + "##k? Well it'll cost you #b" + sCost + " mesos#k. What do you think?");
                                                          selectedMap = U;
                                                        } else if (status == 28) {
                                                          if (cm.getMeso() < sCost) {
