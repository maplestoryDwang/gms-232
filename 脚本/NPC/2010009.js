    ...
    if (status == 1) {
      ...
      if (U == 0) {
        ...
        if (U == 1) {
          ...
          if (U == 2) {
            if (!cm.isLeader()) {
              cm.sendNext("只有族长可以创建家族联盟。");
              cm.dispose();
            } else {
              if (cm.getPlayer().getParty() == null || partymembers == null || partymembers.size() != 2) {
                cm.sendNext("必须要有2名家族长组成组队。");
                cm.dispose();
              } else {
                if (partymembers.get(0).getGuildId() <= 0 || partymembers.get(0).getGuildRank() > 1) {
                  cm.sendOk("You cannot form a Guild Union until you own a guild");
                  cm.dispose();
                } else {
                  if (partymembers.get(1).getGuildId() <= 0 || partymembers.get(1).getGuildRank() > 1) {
                    cm.sendOk("Your party member does not seem to own a guild.");
                    cm.dispose();
                  } else {
                    var V = cm.getGuild(cm.getPlayer().getGuildId());
                    var w = cm.getGuild(partymembers.get(1).getGuildId());
                    if (V.getAllianceId() > 0) {
                      cm.sendOk("You cannot form a Guild Union if you are already affiliated with a different Union.");
                      cm.dispose();
                    } else {
                      if (w.getAllianceId() > 0) {
            ...
            if (U == 3) {
              if (cm.getPlayer().getGuildRank() == 1 && cm.getPlayer().getAllianceRank() == 1) {
                cm.askYesNo("To increase the capacity, you will need to pay 10,000,000 mesos. Are you sure you wish to proceed?");
              } else {
