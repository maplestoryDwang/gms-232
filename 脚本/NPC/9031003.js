...
function action(f, W, U) {
  ...
  if (status == 0) {
    var V = "我是装备制作达人#b埃珅#k。找我有事吗？\r\n";
    if (cm.getPlayer().getProfessionLevel(92020000) > 0) {
      V += "#k#l\r\n#L2##b提升#e装备制作#n等级。#l\r\n#L3#装备制作技术初始化。#k#l";
    } else {
      V += "#L0##b听取有关#e装备制作#n的说明。#l\r\n#L1#学习#e装备制作#n技术。#k#l";
    }
    cm.askMenu(V);
  } else {
    if (status == 1) {
      ...
      if (U == 0) {
        ...
        if (U == 1) {
          if (cm.getLevel() < 30) {
            status = -1;
            cm.sendOk("哎呀……你好像还不够强，还不能学习专业技术。#b至少必须达到30级2转以上，龙神必须3转以上，暗影双刀必须2转+以上#k，才能学习专业技术。等达到条件之后再来找我吧。");
          } else {
            if (cm.getProfessions() >= 3) {
              cm.sendNext("嗯，你好像已经学习了3种专业技术。真想学习的话，就必须先放弃一种技术。");
            } else if (cm.getPlayer().getProfessionLevel(92020000) > 0) {
              cm.sendNext("你已经学习过#e装备制作#n，难道还想学？");
            } else if (cm.getPlayer().getProfessionLevel(92010000) > 0) {
              cm.sendOk("恭喜你成功的学习#e装备制作#n。");
