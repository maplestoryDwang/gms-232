...
function action(f, W, U) {
  ...
  if (status <= V++) {
    ...
    if (status === V++) {
      if (cm.getNumberFromQuestCustomData(28762) > 0) {
      ...
      cm.sendNormalTalk("救命！我骂那些外星人无证采矿，结果被它们关起来了！这不公平啊！", 1, 9201170, false, true);
    } else {
      if (status === V++) {
        ...
        if (status === V++) {
          cm.sendNormalTalk("(锁被你打碎了。)", 3, 9201170, false, true);
        } else {
          if (status === V++) {
            cm.sendNormalTalk("用这返回卷轴回新叶都市去！", 3, 9201170, true, true);
          } else {
            if (status === V++) {
              cm.sendNormalTalk("你救了我！我下次再也不敢骂外星人了！", 1, 9201170, true, true);
            } else {
              if (status === V++) {
                var w = cm.addNumberForQuestCustomData(28749, 1);
                cm.sendNormalTalk("还差 " + (5 - w) + " 个。", 3, 9201167, true, true);
              } else if (status === V++) {
                cm.forceStartQuest(28762, '1');
